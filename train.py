...
def run_policy(env, policy, scaler, logger, episodes):
    ...
    trajectories = []
    global render
    render = mpi_util.chk_cfg_file(render, 'render')
    for e in range(episodes):
        if render: render = mpi_util.chk_cfg_file(render, 'render')
        observes, actions, rewards, unscaled_obs = run_episode(env, policy, scaler, animate=render)
    ...
    logger.log({'_MeanReward':   mpi_util.all_mean( np.mean([t['rewards'].sum() for t in trajectories]) )  , 'Steps': mpi_util.all_sum(total_steps)})
...
def main(env_name, num_episodes, gamma, lam, kl_targ, batch_size, nprocs, policy_hid_list, valfunc_hid_list, gpu_pct, noise_bias):
    """ Main training loop
    ...
    if 1:
        # env = curiosity.inverse_feature_wrapper(env)
        env = curiosity.curiosity_wrapper(env)
        obs_dim = 1 + env.observation_space.shape[0]    # change this to the features size + 1
    scaler = Scaler(obs_dim)
    val_func = NNValueFunction(obs_dim, valfunc_hid_list)
    policy = Policy(obs_dim, act_dim, kl_targ, policy_hid_list, noise_bias)
    # run a few episodes of untrained policy to initialize scaler:
    run_policy(env, policy, scaler, logger, episodes=5)
    episode = 0
    while episode < num_episodes:
        batch_size = mpi_util.chk_cfg_file(batch_size, 'batch_size')
        gamma = mpi_util.chk_cfg_file(gamma, 'gamma')
        lam = mpi_util.chk_cfg_file(lam, 'lam')
        mpi_util.barrier()      # use this to synchonize printing. Comment after debugging
        ...
        mpi_util.rank0_bcast_wts(val_func.sess, val_func.g, label='val')  # doubt if value network is used during rollouts but it only takes a few milliseconds anyhow
        mpi_util.timeit('mpi_util.rank0_bcast_wts(val_func')
        # mpi_util.rank0_bcast_wts(policy.sess, policy.g, 'policy')
        mpi_util.rank0_bcast_wts(policy.sess, policy.g, [policy.means, policy.log_vars], 'policy')
        mpi_util.chk_cfg_file(policy, 'cfg')
        mpi_util.timeit('mpi_util.rank0_bcast_wts(policy')
        if 'curiosity' in str(env): env.end_of_batch()
        mpi_util.timeit('env.end_of_batch')
...
if __name__ == "__main__":
    ...
    # with tf.Session(None, config=mpi_util.tf_config) as sess:
    main(**vars(args))
