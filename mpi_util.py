...
# This is super useful when debugging. It allows print statements to group together
def barrier():
...
def set_wt(sess, graph, var, val):
    ...
    # print(rank,'doing',var,val)
    if not kc in cache:
        with graph.as_default():
            ph = tf.placeholder(var.dtype, shape=val.shape)
            op = var.assign(ph)
            cache[kc] = [ var, ph, op]
    _, ph, op = cache[kc]
    sess.run(op, feed_dict={ph: val})

def traverse(sess, graph, op, spaces=''):
    if op == None: return
    print(spaces,op.name)
    for i in op._inputs:
        # print('input', i.name)
        traverse(sess, graph, i.op, spaces=spaces+'|   ')
    pass

def rank0_bcast_wts(sess, graph, tensor_list=[], label=''):
    # if tensor_list: traverse(sess, graph, tensor_list[0]._op)
    if nworkers == 1: return    # don't bother if only one process
    # chksum(sess, graph, label=label+'before'+str(rank))
    # srt = sorted(graph.get_collection('trainable_variables'),key=lambda x: x.name)
    # print(rank,'srt',srt)
    for var in sorted(graph.get_collection('trainable_variables'),key=lambda x: x.name):
        # barrier()
        # timeit(label+'______'+str(var))
        # print(str(rank)+'rank var', var.name)
        w = sess.run(var)
        # timeit(label+'sess.run'+str(var))
        # wts = bcast(w)
        # print(rank,'loop doing',var)
        wts = MPI.COMM_WORLD.bcast(w)
...
def rank0_bcast_wts_keras(model, label=''):
    if nworkers == 1: return    # don't bother if only one process
    # chksum_keras(model, label=label+'before'+str(rank))
    for layer in model.layers:
        layer_wt_list = [ bcast(wts) for wts in layer.get_weights()]
        if rank != 0:
            layer.set_weights(layer_wt_list)
            # timeit(label+'set_wt'+str(var))
    # chksum_keras(model, label=label+'after'+str(rank))

# def all_avg_wts()
#     tf_set_wts(sess, graph, allmeans(tf.get_wts(sess, graph))
...
def chksum_keras(model, label=''):
    # if nworkers == 1: return    # don't bother if only one process
    print('label in chksum',label)
    chksum = 0
    for layer in model.layers:
        for wts in layer.get_weights():
            chksum += wts.sum()
    print('CHKSUM rank:',rank, 'nworkers',nworkers, 'label',label,'chksum', chksum)
...
def timeit(label):
    ...
    if '------' in label:    # bunch of hyphens indicates top of the loop
        if loop_start_time > 0:
            print(str(rank)+'timed-'+label+' TimeofLastLoop',thistime-loop_start_time,label)    # print the time of the last loop
...
import numbers
# This is used so we can change the attributes of an object as the program is running
# by writing to a file
# call this somewhere in your main loop
#      mpi_util.chk_cfg_file(policy, 'cfg')
#  then use this in a terminal window      echo 'sigma .1' > cfg    to change the value of self.sigma
# or   touch cfg    to just print what the current values are
# accepts a scalar, dict or object. Return the new value(if scalar) or original dict or object with new value in it
def chk_cfg_file(self_or_dict_or_value, filename='cfg'):
    if isinstance(self_or_dict_or_value, numbers.Number):
        d = {filename: self_or_dict_or_value}
    elif isinstance(self_or_dict_or_value, dict): d = self_or_dict_or_value
    elif isinstance(self_or_dict_or_value, object): d = self_or_dict_or_value.__dict__
    key_val = []
    if rank==0:
        if os.path.isfile(filename):
            with open(filename) as f:
                ...
                try:
                    if len(strr)>0:
                        m=re.split('\s+', strr, maxsplit=1)
                        if len(m) != 2:
                            print('the cfg file should be     key value      on one line')
                        else:
                            key = m[0]
                            if key in d:
                                oldval = d[key]
                                print("  the oldval was",d[key])
                                if type(oldval) is int:
                                    val = int(m[1])
                                else:
                                    val = float(m[1])
                            else:
                                print("  there was no previous value for",key)
                                oldval = None
                                val = float(m[1])       # if no old val, just assume float
                            print('  found_new_cfg ',key, val, 'oldval:',oldval)
                            key_val = [key, val]
                except: pass
            os.remove(filename)
    key_val = MPI.COMM_WORLD.bcast(key_val)
    if key_val:
        k, v = key_val
        print(str(rank)+'rank setting',k,'to',v)
        d[k] = v
    if isinstance(self_or_dict_or_value, numbers.Number):
        return d[filename]
    else:
...
# This generates True if so many seconds have passed since last time it was true. Useful for env.render among others
time_last_noted = 0
last_call_was_true = False
def time_passed(secs):
    global time_last_noted, last_call_was_true
    if rank==0:     # can only have one decision maker on this
        if (time.time() - time_last_noted) > secs:
            time_last_noted = time.time()
            last_call_was_true = True
            return True
    last_call_was_true = False
    return False
def time_finished():        # optionally call this after the render is finished such that the original specified time is the time interval between finish and start
    global time_last_noted, last_call_was_true
    if rank==0 and last_call_was_true: time_last_noted = time.time()
